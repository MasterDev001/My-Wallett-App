...
class BorrowUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        if (personCurrencyRepository.isPersonCurrenciesCurrencyExists(
            ...
            val newPersonCurrency = personCurrencyRepository.getPersonCurrency(
                personData.id,
                currencyData.id
            )
            val newBalance =
                newPersonCurrency.currencyBalance - amount.toDouble()
            if (newBalance == 0.0) {
                personCurrencyRepository.deletePersonCurrency(newPersonCurrency.id)
            } else {
                personCurrencyRepository.updatePersonCurrency(
                    newPersonCurrency.copy(
