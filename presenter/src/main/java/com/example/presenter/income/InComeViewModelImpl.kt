        ...
        when (intent) {
            ...
            is InComeContract.Intent.IncomeMoney -> {
                coroutineScope.launch(Dispatchers.IO) {
                    ...
                    val c = intent.wallet.walletOwnerDataList.walletOwnerData
                    if (walletsUseCase.isCurrencyIdExistsInWalletUseC.invoke(
                            intent.wallet.id,
                            intent.currencyData.id
                        )
                    ) {
                        c.forEach {
                            if (it.id == intent.currencyData.id) {
                                it.currencyBalance += intent.amount
                            }
                        }
                    } else {
                        c.add(
                    ...
                    walletsUseCase.updateWalletUseC.invoke(
                        intent.wallet.copy(walletOwnerDataList = WalletOwnerDataList(c))
                    )
                }
            }
        }
